...
class VGG(object):
  ...
  def fit(self, X_train, y_train, X_val, y_val, sample_weight=None):
    ...
    es = keras.callbacks.EarlyStopping(monitor='val_loss', mode='auto', verbose=1, patience=100)
    reduceLR = keras.callbacks.ReduceLROnPlateau(monitor='loss', patience=30, factor=0.1, verbose=1)
