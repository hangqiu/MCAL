...
class Entropy(SamplingMethod):
  ...
  def select_batch_(self, model, already_selected, N, **kwargs):
    ...
    inf_t = time.time()
    entropy = (np.log(distances) * distances).sum(axis=1) * -1.
    ...
    inf_t = time.time()
    rank_ind = entropy.argsort(axis=0)[::-1]
    ...
    if N!=0:
      print("Evaluated entropy metric 0-{}: [{},{}]".format(N, entropy[rank_ind[0]], entropy[rank_ind[N-1]]))
    else:
